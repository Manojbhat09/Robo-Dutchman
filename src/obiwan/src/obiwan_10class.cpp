...
void get_rect_dimentions(cv::Mat& img, float bbox[4], float &left, float &top, float &width, float &height) {
    int l, r, t, b;
    float r_w = INPUT_W / (img.cols * 1.0);
    float r_h = INPUT_H / (img.rows * 1.0);
    if (r_h > r_w) {
...
ICudaEngine* createEngine(unsigned int maxBatchSize, IBuilder* builder, DataType dt) {
    ...
    std::map<std::string, Weights> weightMap = loadWeights("../valve_detect.wts"); //yolov3-spp_ultralytics68.wts
    Weights emptywts{DataType::kFLOAT, nullptr, 0};
    ...
    IConvolutionLayer* conv88 = network->addConvolution(*lr87->getOutput(0), 45, DimsHW{1, 1}, weightMap["module_list.88.Conv2d.weight"], weightMap["module_list.88.Conv2d.bias"]);
    assert(conv88);
    ...
    IConvolutionLayer* conv100 = network->addConvolution(*lr99->getOutput(0), 45, DimsHW{1, 1}, weightMap["module_list.100.Conv2d.weight"], weightMap["module_list.100.Conv2d.bias"]);
    assert(conv100);
    ...
    IConvolutionLayer* conv112 = network->addConvolution(*lr111->getOutput(0), 45, DimsHW{1, 1}, weightMap["module_list.112.Conv2d.weight"], weightMap["module_list.112.Conv2d.bias"]);
    assert(conv112);
...
void doInference(IExecutionContext& context, float* input, float* output, int batchSize) {
    ...
    void* buffers[2];
    if(DEBUG) std::cout<<"[DEBUG] Starting engine \n";
    // In order to bind the buffers, we need to know the names of the input and output tensors.
    // Note that indices are guaranteed to be less than IEngine::getNbBindings()
    const int inputIndex = engine.getBindingIndex(INPUT_BLOB_NAME);
    const int outputIndex = engine.getBindingIndex(OUTPUT_BLOB_NAME);
    if(DEBUG) std::cout<<"[DEBUG] Starting cudaMalloc \n";
    // Create GPU buffers on device
    CHECK(cudaMalloc(&buffers[inputIndex], batchSize * 3 * INPUT_H * INPUT_W * sizeof(float)));
    CHECK(cudaMalloc(&buffers[outputIndex], batchSize * OUTPUT_SIZE * sizeof(float)));
    if(DEBUG) std::cout<<"[DEBUG] Starting cudaStreamCreate \n";
    // Create stream
    cudaStream_t stream;
    CHECK(cudaStreamCreate(&stream));
    if(DEBUG) std::cout<<"[DEBUG] Starting cudaMemcpyAsync \n";
    // DMA input batch data to device, infer on the batch asynchronously, and DMA output back to host
    CHECK(cudaMemcpyAsync(buffers[inputIndex], input, batchSize * 3 * INPUT_H * INPUT_W * sizeof(float), cudaMemcpyHostToDevice, stream));
    context.enqueue(batchSize, buffers, stream, nullptr);
    CHECK(cudaMemcpyAsync(output, buffers[outputIndex], batchSize * OUTPUT_SIZE * sizeof(float), cudaMemcpyDeviceToHost, stream));
    if(DEBUG) std::cout<<"[DEBUG] Starting cudaStreamSynchronize \n";
    cudaStreamSynchronize(stream);
    if(DEBUG) std::cout<<"[DEBUG] Starting cudaStreamDestroy \n";
...
static void setup_inference(){
    cudaSetDevice(DEVICE);
    // create a model using the API directly and serialize it to a stream
    char *trtModelStream{nullptr};
    size_t size{0};
    std::string filepath = fs::current_path();
    if(DEBUG) std::cout<<"[DEBUG] Current path to include engine is "<<filepath.c_str()<<"\n";
    
    if(DEBUG) std::cout<<"[DEBUG] Valve eng\n";
    std::ifstream file(filepath+"/"+"valve_detect.engine", std::ios::binary); //yolov3-spp.engine
    if (file.good()) {
        ...
        file.close();
        if(DEBUG) std::cout<<"[DEBUG] Imported Engine \n";
    }
    if(DEBUG) std::cout<<"[DEBUG] Engine done\n";
    PluginFactory pf;
    runtime = createInferRuntime(gLogger);
    assert(runtime != nullptr);
    engine = runtime->deserializeCudaEngine(trtModelStream, size, &pf);
    assert(engine != nullptr);
    if(DEBUG) std::cout<<"[DEBUG] Context execution\n";
    context = engine->createExecutionContext();
    assert(context != nullptr);
    fcount = 0;
    if(DEBUG) std::cout<<"[DEBUG] Returning\n";
}



static int put_destroy_engine(){
    context->destroy();
    engine->destroy();
    runtime->destroy();
    return 0;
...
std::vector<float> get_inference(cv::Mat &input_image){
    
    cv::rotate(input_image,input_image, cv::ROTATE_90_COUNTERCLOCKWISE);
//     std::vector<std::vector<float>> return_vec; 
    std::vector<float> return_vec; 
    if(DEBUG) std::cout<<"[DEBUG] Starting function \n";
    float data[3 * INPUT_H * INPUT_W];
//     if (input_image.empty()) return return_vec;
    cv::Mat pr_img = preprocess_img(input_image);
    ...
    if(DEBUG) std::cout<<"[DEBUG] Starting inference \n";
    // Run inference
    auto start = std::chrono::system_clock::now();
    doInference(*context, data, prob, 1);
    if(DEBUG) std::cout<<"[DEBUG] Starting nms \n";
...
//     return_vec.resize(5*res.size()); //return_vec.resize(res.size());
    
    if(DEBUG) std::cout<<"[DEBUG] Completed inference, storing data \n";
    
    for (size_t j = 0; j < res.size(); j++) {
        // return_vec[j].resize(5);
        float *p = (float*)&res[j];
        float left, right, width, height;
        get_rect_dimentions(input_image, res[j].bbox, left, right, width, height);
//         cv::Rect r = get_rect(input_image, res[j].bbox);
//         cv::rectangle(input_image, r, cv::Scalar(0x27, 0xC1, 0x36), 2);
//         cv::putText(input_image, std::to_string((int)res[j].class_id), cv::Point(r.x, r.y - 1), cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
//         cv::putText(input_image, std::to_string(fps), cv::Point(r.x + res[j].bbox[2]-1, r.y ),cv::FONT_HERSHEY_PLAIN, 1.2, cv::Scalar(0xFF, 0xFF, 0xFF), 2);
        float class_id = (float)res[j].class_id;
        if(DEBUG) std::cout<<"[DEBUG] class is "<< class_id <<"\n";
        return_vec.push_back(class_id); //return_vec[j]
        return_vec.push_back(left);
        return_vec.push_back(right);
        return_vec.push_back(width);
        return_vec.push_back(height);
    }
    
    if(DEBUG) std::cout<<"[DEBUG] Completed storage, returning \n";
    //cv::imwrite("_" + f, input_image);
    return return_vec;
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  ...
  // Initialize the engine for inference 
  if(first_time==0){
    if(DEBUG) std::cout<<"[DEBUG] Starting setup\n";
    setup_inference();
    if(DEBUG) std::cout<<"[DEBUG] Finished setup\n";
    first_time = 1;
  } 
  // Try the engine for the infeence and return error if any engine found 
  try
  {
//     updateAsk = 1;
    if(updateAsk == 1){
        cv::Mat input_image;
        if(DEBUG) std::cout<<"[DEBUG] Imported Image \n";
        input_image = cv_bridge::toCvShare(msg, "bgr8")->image;
        std::vector<float> localizer_data = get_inference(input_image);
        if(DEBUG) std::cout<<"[DEBUG] Imported Image \n";

        /* Publisher stream */
        std::stringstream ss;
        for(auto it=localizer_data.begin(); it!=localizer_data.end(); it++){
    //         std::cout << (float)*it<<"\t";
            ss << (float)*it << ",";
        }
        std::cout<<"\n";
        out_msg.data = ss.str();
        if(DEBUG) std::cout<<"[DEBUG] Publish \n";
        pub.publish(out_msg);
        /* ROS_INFO("%s", msg.data.c_str()); */
        if(DEBUG) std::cout<<"[DEBUG] Published message \n";
        updateAsk = 0;
...
//         cv::waitKey(2);
        ...
        updateAsk = 0;
//         if(DEBUG) std::cout<<"[DEBUG] No update asked, skipping... \n";
    }
    ros::spinOnce();
  }
...
void updateCallback(const std_msgs::String::ConstPtr & msg){
    std::string string_data = msg->data;
    if(std::stoi(string_data) == 1)
        updateAsk = 1;
    else 
        updateAsk = 0;
}
...
int main(int argc, char** argv) {
    ...
    ros::init(argc, argv, "only_hope");
    ros::NodeHandle nh;
    
    nh.getParam("/obiwancpp/param_debug", param_debug); 
    if(param_debug == 1) DEBUG = 1;
    
    /* Adding publisher */
    pub = nh.advertise<std_msgs::String>("/localizer/target/detect", 1000);
    /* Starting CV Window */
    image_transport::ImageTransport it(nh);
    if(DEBUG) std::cout<<"[DEBUG] Starting subscribe\n";
    /* Subscriber */
    image_transport::Subscriber sub_image = it.subscribe("/camera/color/image_raw", 1, imageCallback);
    ros::Subscriber sub_ask = nh.subscribe("/localizer/target/update_ask_obiwan", 1, updateCallback);
